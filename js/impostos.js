...
const createProduct = async () => {
    ...
    await fetch('http://localhost:8080/tax-type-product/', {
        method: 'POST',
        headers: {
    ...
    .then(data => {
        showLog('Imposto criado com sucesso', 'text-bg-success');
    })
    .catch(error => {
        showLog('Erro ao tentar cadastrar o imposto', 'text-bg-danger');
    });
...
const showUpdateModalProduct = async (productId) => {
    ...
    valueProductModal.value = product.data.percentage;
...
const updateProduct = async (productId) => {
    ...
    await fetch(`http://localhost:8080/tax-type-product/${productId}/`, {
        method: 'PUT',
        headers: {
            'Content-Type': 'application/json'
        },
        body: JSON.stringify({
            ...
            percentage : valueProductModal.value,
            type_product_id : typeProductModal.value
        })
    })
    .then(response => response.json())
    .then(data => {
        showLog('Imposto atualizado com sucesso', 'text-bg-success');
    })
    .catch(error => {
        showLog('Erro ao tentar atualizar o imposto', 'text-bg-danger');
    }); 
...
const getProduct = async (productId) => {
    let response = await fetch(`http://localhost:8080/tax-type-product/${productId}/`);
    return await response.json();
}
...
const getProducts = async () => {
    ...
    let newBody = '';
    let response = await fetch(`http://localhost:8080/tax-type-product/`);
